...
extension Marconi {
    public class PlayerObserver: NSObject {
        ...
        private var _playbackStatusObserver: NSKeyValueObservation?
    
        private var _stateMachine: StateMachine = .init()
        
        private func _observeBuffering(_ playerItem: AVPlayerItem?) {
            _playbackBufferEmptyObserver = playerItem?.observe(\.isPlaybackBufferEmpty, options: [.old, .new]) { (old, new) in
                // show buffering
                
                print("buffering... from \(old)->\(new)")
            }
            
            _playbackLikelyToKeepUpKeyPathObserver = playerItem?.observe(\.isPlaybackLikelyToKeepUp, options: [.old, .new]) { (old, new) in
                // hide buffering
                print("buffering ends.. from \(old)->\(new)")
            }
            
            _playbackBufferFullObserver = playerItem?.observe(\.isPlaybackBufferFull, options: [.old, .new]) { (old, new) in
                print("buffering is hidden... from \(old)->\(new)")
                // hide buffering
            }
        }
        
        private func _observeStatus(_ playerItem: AVPlayerItem?) {
            _playbackStatusObserver = playerItem?.observe(\.status, options:  [.new, .old]) { (playerItem, change) in
                print("current item status is \(playerItem.status.rawValue)")
            }
        }
